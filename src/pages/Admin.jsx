          ...
          {[
            ...
            <button key={o.key} onClick={() => setOnglet(o.key)}
              className={`py-4 text-sm font-medium border-b-2 transition ${
                onglet === o.key ? 'border-[#2d6a4f] text-[#2d6a4f]' : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}>
              {o.label}
            </button>
          ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              {[
                ...
                <div key={i} className="bg-white rounded-2xl p-5 shadow-sm">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg mb-3 ${s.color}`}>{s.icon}</div>
                  ...
                  <tbody>
                    {associations.map((a, i) => (
                      <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                        ...
                        <td className="py-3">
                          <button onClick={() => handleSupprimerAssociation(a.id, a.nom)}
        ...
        {onglet === 'validations' && (
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-gray-800">Demandes d'inscription en attente ({pending.length})</h2>
            </div>
            {pending.length === 0 ? (
              <div className="bg-white rounded-2xl p-10 text-center shadow-sm text-gray-400">Aucune demande en attente</div>
            ) : (
                  ...
                  <div key={i} className="bg-white rounded-2xl p-5 shadow-sm">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        {v.site_web && <div><p className="text-xs font-semibold text-gray-500 uppercase mb-1">Site web</p><p className="text-gray-700">{v.site_web}</p></div>}
                        {v.telephone && <div><p className="text-xs font-semibold text-gray-500 uppercase mb-1">Téléphone</p><p className="text-gray-700">{v.telephone}</p></div>}
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => handleApprouver(v.id)}
                        className="flex-1 bg-[#2d6a4f] text-white text-sm px-4 py-2 rounded-full hover:bg-[#245a42] transition">
                        ✓ Approuver
                      </button>
                      <button onClick={() => handleRejeter(v.id)}
              ...
              {projets.map((p, i) => (
                <div key={i} className="bg-white rounded-2xl p-5 shadow-sm">
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => handleSupprimerProjet(p.id)}
