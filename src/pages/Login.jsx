...
export default function Login() {
  ...
  const roles = [
    ...
    { value: 'association', icon: '🤝', label: 'Association', desc: 'Je représente une association diaspora', redirect: '/inscrire-association' },
              ...
              {roles.map((r) => (
                r.redirect ? (
                  <Link
                    key={r.value}
                    to={r.redirect}
                    className="flex items-center gap-3 p-3 rounded-xl border-2 text-left transition border-gray-200 hover:border-[#2d6a4f] hover:bg-[#f0f7f4]"
                  >
                    ...
                    <span className="ml-auto text-xs text-[#2d6a4f] font-medium">Formulaire dédié →</span>
                  </Link>
                ) : (
                  <button
                    key={r.value}
